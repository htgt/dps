...
my $crispr_type_param = 'single';
my @region_groups_param;
my $output_param;
...
sub usage_message {
    LOGDIE << 'END_USAGE';
Usage: lookup_crisprs.pl <loci_file.csv>
    --file=< locus_file.csv >
    --species=[ human | mouse ]
...
unless ($csv_file_param) {
    FATAL 'Specify input file';
    $doomed++;
}
unless ($species_param) {
    FATAL 'Specify species';
    $doomed++;
}
unless ($crispr_type_param) {
    FATAL 'Specify crispr_type';
    $doomed++;
}
unless (@region_groups_param) {
    FATAL 'Specify region_groups';
    $doomed++;
}
unless ($output_param) {
    FATAL 'Specify output';
    $doomed++;
}

usage_message() if $doomed;

my $SPECIES_ID;

if ( $species_param eq 'human') {
    $SPECIES_ID = 1;
}
elsif ( $species_param eq 'mouse') {
    $SPECIES_ID = 2;
}


#should be pair or crispr
my $MODE = $crispr_type_param;
die "Mode must be 'single' or 'pair'" unless $MODE eq 'single' or $MODE eq 'pair';

my $w = WGE::Model::DB->new;
my $pair_finder = WGE::Util::FindPairs->new( schema => $w->schema );

my $csv = Text::CSV->new();
open ( my $fh, '<', $csv_file_param ) or die( "Cannot open $csv_file_param: " . $! );
...
#my @region_groups = qw( pre post ); #this shuold be loxp cassette/pre post/whatever

my @region_groups = @region_groups_param;
...
my $output_fh = IO::File->new( $output_param . "_$MODE.csv" , 'w' );
...
sub score_off_target_summary {
    ...
    if ( ! $text){
        DEBUG 'No off target summary available for these crisprs';
        $DB::single=1;
        return 0;
    }
    #load yaml string
...
Where there is no design available and input is the co-ordinates of exons, mode of operation is to set:

@region_groups = qw( pre post );

then generate a CSV from your list of exon ids that has the fields
pre_chr_name, pre_chr_start, pre_chr_end and post_chr_name,
post_chr_start, post_chr_end. It will then find the best 2 crispr sites
for those regions.

The other script is better suited to finding crisprs _inside_ exons for
a gene list, whereas this one is for any region you want.
