...
while ( my $line = <> ) {
    chomp $line;
    my ( $marker_symbol, $exon ) = split /,/, $line;

    next if $marker_symbol =~ /^#/; #allow commented lines
    print STDERR "Processing ($exon) $marker_symbol\n";
    my $exon = $exon_adaptor->fetch_by_stable_id($exon);
    push @exons, $exon;
}
...
for my $exon ( @exons ) {
    say STDERR "Processed $count exons" if ++$count % 1000 == 0;

    if ( $exon->seq_region_name !~ /^(?:\d+|X|Y|MT)$/ ) {
        say STDERR "Skipping gene on " . $exon->seq_region_name;
        next;
    }
