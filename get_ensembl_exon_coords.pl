...
$DB::single=1;
    ...
    $ov->{pre_chr_start} = $exon->start - $pre_region_param;
            ...
            if (my $new_left = is_left_overlap( $trans_exon, $ov))
             {
                ...
                say '+ old post_chr_start was: ' . $ov->{post_chr_start};
                $ov->{post_chr_start} = $new_right - 1;
                say '+ new post_chr_start is:  ' . $ov->{post_chr_start};
            }
        }
    }
