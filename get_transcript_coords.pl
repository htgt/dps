...
$DB::single=1;
    my $transcript_ar = $tr_adaptor->fetch_all_by_exon_stable_id( $exon_id );
    my ($pre_chr_name, $pre_chr_start, $pre_chr_end);
    my ($post_chr_name, $post_chr_start, $post_chr_end);
    my ($exon_chr_start, $exon_chr_end);

    foreach my $transcript ( @$transcript_ar ) {
    	my $count_transcripts++;
    	say 'Exon: ' . $exon_id . ': transcript: ' . $count_transcripts;
    	my $transformed = $transcript->transform('chromosome');
    	$transformed or die "No co-ordinate transform available!";

	 	$pre_chr_name = $transcript->slice->seq_region_name();
	 	$exon_chr_start = $transcript->start;
	    $pre_chr_start = $transcript->start - 151;
	    $pre_chr_end = $transcript->start - 1;

	    $exon_chr_end = $transcript->end;
	    $post_chr_name = $transcript->slice->seq_region_name;
	    $post_chr_start = $transcript->end + 1;
	    $post_chr_end = $transcript->end + 150;
